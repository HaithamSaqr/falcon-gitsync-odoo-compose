...
Production-ready Odoo 16 deployment with Docker Compose, featuring optional automatic addons synchronization from GitHub via git-sync.
...
- **Supports Odoo 16** with PostgreSQL 15
...
```bash
curl -sSL https://raw.githubusercontent.com/HaithamSaqr/falcon-gitsync-odoo-compose/16.0/run.sh | bash -s -- \
  --destination /opt/odoo16 \
  --port 10016 \
  --chat 20016
```
...
# Instance 1
curl -sSL https://raw.githubusercontent.com/HaithamSaqr/falcon-gitsync-odoo-compose/16.0/run.sh | bash -s -- \
  --destination /opt/odoo16-client1 \
  --port 10016 \
  --chat 20016

# Instance 2
curl -sSL https://raw.githubusercontent.com/HaithamSaqr/falcon-gitsync-odoo-compose/16.0/run.sh | bash -s -- \
  --destination /opt/odoo16-client2 \
  --port 11019 \
  --chat 21019
```
...
|-----------|----------|-------------|
| `--destination` | Yes | Installation directory (e.g., `/opt/odoo16`) |
| `--port` | Yes | Odoo web port (e.g., `10016`) |
| `--chat` | Yes | Odoo live chat / websocket port (e.g., `20016`) |
...
All commands should be run from the installation directory (e.g., `/opt/odoo16`).
...
server {
    ...
    location /websocket {
        proxy_pass http://127.0.0.1:20016;
