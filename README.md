...
Production-ready Odoo 18 deployment with Docker Compose, featuring optional automatic addons synchronization from GitHub via git-sync.
...
- **Supports Odoo 18** with PostgreSQL 17
...
```bash
curl -sSL https://raw.githubusercontent.com/HaithamSaqr/falcon-gitsync-odoo-compose/18.0/run.sh | bash -s -- \
  --destination /opt/odoo18 \
  --port 10018 \
  --chat 20018
```
...
# Instance 1
curl -sSL https://raw.githubusercontent.com/HaithamSaqr/falcon-gitsync-odoo-compose/18.0/run.sh | bash -s -- \
  --destination /opt/odoo18-client1 \
  --port 10018 \
  --chat 20018

# Instance 2
curl -sSL https://raw.githubusercontent.com/HaithamSaqr/falcon-gitsync-odoo-compose/18.0/run.sh | bash -s -- \
  --destination /opt/odoo18-client2 \
  --port 11019 \
  --chat 21019
```
...
|-----------|----------|-------------|
| `--destination` | Yes | Installation directory (e.g., `/opt/odoo18`) |
| `--port` | Yes | Odoo web port (e.g., `10018`) |
| `--chat` | Yes | Odoo live chat / websocket port (e.g., `20018`) |
...
All commands should be run from the installation directory (e.g., `/opt/odoo18`).
...
server {
    ...
    location /websocket {
        proxy_pass http://127.0.0.1:20018;
