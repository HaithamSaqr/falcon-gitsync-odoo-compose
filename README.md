...
Production-ready Odoo 20 deployment with Docker Compose, featuring optional automatic addons synchronization from GitHub via git-sync.
...
- **Supports Odoo 20** with PostgreSQL 18
...
```bash
curl -sSL https://raw.githubusercontent.com/HaithamSaqr/falcon-gitsync-odoo-compose/20.0/run.sh | bash -s -- \
  --destination /opt/odoo20 \
  --port 10020 \
  --chat 20020
```
...
# Instance 1
curl -sSL https://raw.githubusercontent.com/HaithamSaqr/falcon-gitsync-odoo-compose/20.0/run.sh | bash -s -- \
  --destination /opt/odoo20-client1 \
  --port 10020 \
  --chat 20020

# Instance 2
curl -sSL https://raw.githubusercontent.com/HaithamSaqr/falcon-gitsync-odoo-compose/20.0/run.sh | bash -s -- \
  --destination /opt/odoo20-client2 \
  --port 11019 \
  --chat 21019
```
...
|-----------|----------|-------------|
| `--destination` | Yes | Installation directory (e.g., `/opt/odoo20`) |
| `--port` | Yes | Odoo web port (e.g., `10020`) |
| `--chat` | Yes | Odoo live chat / websocket port (e.g., `20020`) |
...
All commands should be run from the installation directory (e.g., `/opt/odoo20`).
