...
Production-ready Odoo 14 deployment with Docker Compose, featuring optional automatic addons synchronization from GitHub via git-sync.
...
- **Supports Odoo 14** with PostgreSQL 13
...
```bash
curl -sSL https://raw.githubusercontent.com/HaithamSaqr/falcon-gitsync-odoo-compose/14.0/run.sh | bash -s -- \
  --destination /opt/odoo14 \
  --port 10014 \
  --chat 20014
```
...
# Instance 1
curl -sSL https://raw.githubusercontent.com/HaithamSaqr/falcon-gitsync-odoo-compose/14.0/run.sh | bash -s -- \
  --destination /opt/odoo14-client1 \
  --port 10014 \
  --chat 20014

# Instance 2
curl -sSL https://raw.githubusercontent.com/HaithamSaqr/falcon-gitsync-odoo-compose/14.0/run.sh | bash -s -- \
  --destination /opt/odoo14-client2 \
  --port 11019 \
  --chat 21019
```
...
|-----------|----------|-------------|
| `--destination` | Yes | Installation directory (e.g., `/opt/odoo14`) |
| `--port` | Yes | Odoo web port (e.g., `10014`) |
| `--chat` | Yes | Odoo live chat / websocket port (e.g., `20014`) |
...
All commands should be run from the installation directory (e.g., `/opt/odoo14`).
