...
Production-ready Odoo 15 deployment with Docker Compose, featuring optional automatic addons synchronization from GitHub via git-sync.
...
- **Supports Odoo 15** with PostgreSQL 14
...
```bash
curl -sSL https://raw.githubusercontent.com/HaithamSaqr/falcon-gitsync-odoo-compose/15.0/run.sh | bash -s -- \
  --destination /opt/odoo15 \
  --port 10015 \
  --chat 20015
```
...
# Instance 1
curl -sSL https://raw.githubusercontent.com/HaithamSaqr/falcon-gitsync-odoo-compose/15.0/run.sh | bash -s -- \
  --destination /opt/odoo15-client1 \
  --port 10015 \
  --chat 20015

# Instance 2
curl -sSL https://raw.githubusercontent.com/HaithamSaqr/falcon-gitsync-odoo-compose/15.0/run.sh | bash -s -- \
  --destination /opt/odoo15-client2 \
  --port 11019 \
  --chat 21019
```
...
|-----------|----------|-------------|
| `--destination` | Yes | Installation directory (e.g., `/opt/odoo15`) |
| `--port` | Yes | Odoo web port (e.g., `10015`) |
| `--chat` | Yes | Odoo live chat / websocket port (e.g., `20015`) |
...
All commands should be run from the installation directory (e.g., `/opt/odoo15`).
